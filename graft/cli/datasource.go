...
func DatasourceCommand(gClient grafana.Client) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "Create and delete Grafana datasources. All the changes are reflected in the state file.",
	}
...
func createDataSourceCommand(gClient grafana.Client) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Create a datasource in Grafana and add it to the state file. The datasource-uid is a required argument.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return currentState.AddDataSource(dataSource)
		},
	}
...
func deleteDataSourceCommand(gClient grafana.Client) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "Delete a datasource from Grafana and remove it from the state file. The datasource-uid is a required argument.",
		Args:  cobra.ExactArgs(1),
		PreRun: func(cmd *cobra.Command, args []string) {
			datasourceUID = args[0]
		},
		RunE: func(cmd *cobra.Command, args []string) error {
			dataSource, err := currentState.GetDatasource(datasourceUID)
			if err != nil {
				return err
			}
			...
			return currentState.DeleteDatasource(datasourceUID)
		},
	}
	return cmd
}
