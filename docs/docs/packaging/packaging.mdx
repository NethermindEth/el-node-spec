...
The Tap should contain a `README.md` and `checksum.txt` files, and a folder with the contents described [here](#structure) named after the git tag that represents the release of a new package. The following directory structure can be used as reference:
