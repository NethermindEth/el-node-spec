...
The Eigen Layer Middleware Specification is designed for **Linux** systems. The packaging structure and its functionality rely on the use of Docker and Docker Networking, including host networking, which needs to be better supported on Windows or Mac OS.
