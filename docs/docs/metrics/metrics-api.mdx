...
The EigenLayer Middleware API provides monitoring and common metrics for Nodes designed for the EigenLayer protocol.
...
Returns the health status of the given service in HTTP status codes. The service ID is the service ID returned by the `/eigen/v1/services` endpoint. The service is considered healthy if the service is registered with the Node and the service is currently online.
